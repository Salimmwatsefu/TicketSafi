...
import { Ticket, Lock, Loader2, User, Briefcase, Key } from 'lucide-react';
...
const LoginPage = () => {
  ...
  useEffect(() => {
      if (isOrganizer && !invitationCode) {
           navigate('/organizer/gate/login', { replace: true });
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (isEmail) loginPayload.email = formData.identifier;
        else loginPayload.username = formData.identifier;

        await login(loginPayload);
        await handleRoleBasedRedirect();
    } catch (err: any) {
        if (err.response?.data?.non_field_errors) {
            setError(<span>{err.response.data.non_field_errors[0]} <Link to="/forgot-password" className="underline font-bold hover:text-white">Reset Password</Link></span>);
        } else {
       ...
       <div className="w-full max-w-md bg-surface/50 backdrop-blur-xl border border-white/10 p-8 rounded-3xl shadow-2xl">
           ...
           <form onSubmit={handleSubmit} className="space-y-5">
               <div>
                   ...
                   <div className="relative">
                       ...
                       <input type="text" className={`w-full bg-black/20 border border-white/10 rounded-xl py-3 pl-12 text-white outline-none focus:ring-2 ${isOrganizer ? 'focus:ring-secondary/50' : 'focus:ring-primary/50'}`} placeholder="username or email" value={formData.identifier} onChange={(e) => setFormData({...formData, identifier: e.target.value})} required />
                   </div>
               </div>
               <div>
                   ...
                   <div className="relative">
                       ...
                       <input type="password" className={`w-full bg-black/20 border border-white/10 rounded-xl py-3 pl-12 text-white outline-none focus:ring-2 ${isOrganizer ? 'focus:ring-secondary/50' : 'focus:ring-primary/50'}`} placeholder="••••••••" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} required />
                   </div>
               </div>
               <div className="flex justify-end">
                   <Link to="/forgot-password" className="text-xs font-bold text-zinc-400 hover:text-white transition-colors">Forgot Password?</Link>
               </div>
           ...
           <div className="mt-8 text-center">
               <p className="text-zinc-400 text-sm">
                   ...
                   <Link to={config.registerLink + (invitationCode ? `?code=${invitationCode}` : '')} className={`${config.themeColor} font-bold hover:underline`}>
